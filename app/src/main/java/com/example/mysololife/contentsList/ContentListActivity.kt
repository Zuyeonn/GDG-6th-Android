...
class ContentListActivity : AppCompatActivity() {
    ...
    val bookmarkIdList= mutableListOf<String>() // 북마크 id를 저장 리스트, dataModel.key 에서 나오는 data 저장
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val itemsKeyList = ArrayList<String>() // 키값 저장.

        rvAdapter = ContentRVAdapter(baseContext, items, itemsKeyList, bookmarkIdList) // rvAdapter 생성

        // Write a message to the database
        val database = Firebase.database

        val category = intent.getStringExtra("category") // 카테고리 값 받아옴

        if(category == "category1") {   // 카테고리1 일때

            myRef = database.getReference("contents")

        } else if (category == "category2") {   // 카테고리2 일 때
        ...
        val postListener = object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                for (dataModel in dataSnapshot.children){
                    ...
                    items.add(item!!)
                    itemsKeyList.add(dataModel.key.toString()) // 키 값을 다 넣어주고
                }
                rvAdapter.notifyDataSetChanged() // 데이터를 다 받아오고 나서는 어댑터를 새롭게 동기화해줘라
        ...
        val rv : RecyclerView = findViewById(R.id.rv) // 어댑터를 만들고 어댑터와 연결해줌

        rv.adapter = rvAdapter // rvAdapter가 내가 만든 어댑터라고 알려줌
        rv.layoutManager = GridLayoutManager(this, 2) // 두 줄로
    ...
    private fun getBookmarkData(){
        ...
        val postListener = object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                for (dataModel in dataSnapshot.children){
                    bookmarkIdList.add(dataModel.key.toString()) // 리스트에 북마크 id저장
                }
...
//        rvAdapter.itemClick = object : ContentRVAdapter.ItemClick {     // 아이템 클릭
//            override fun onClick(view: View, position: Int) {
//
//                Toast.makeText(baseContext, items[position].title, Toast.LENGTH_LONG).show() // 토스트 메시지: items에 있는 title을 가져온다
//
//                // ContentShowActivity로 넘겨준다.
//                val intent = Intent(this@ContentListActivity, ContentShowActivity::class.java)
//                intent.putExtra("url", items[position].webUrl)  // url이라는 이름 안에 items에 있는 선택된(클릭된) 값에 webUrl을 넘긴다.
