...
class BoardWriteActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.writeBtn.setOnClickListener {
            ...
            // 파이어베이스 strage에 이미지를 저장하고 싶습니다.

            // 만약에 내가 게시글을 클릭했을 때, 게시글에 대한 정보를 받아와야 하는데
            // 이미지 이름에 대한 정보를 모르기 때문에
            // 이미지 이름을 문서의 key값으로 해줘서 이미지에 대한 정보를 찾기 쉽게 해놓음.

            val key = FBRef.boardRef.push().key.toString()  // 생성되기 전에 키값을 받아옴. 문서의 임의의 값을 이미지 이름으로
        ...
        binding.imageArea.setOnClickListener {  // imageArea 버튼을 클릭하면
    ...
    private fun imageUpload(key : String) {  // 이미지 업로드
        ...
        val mountainsRef = storageRef.child(key + ".png")  // 파일이름을 키값으로
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(resultCode == RESULT_OK && requestCode == 100) {
            binding.imageArea.setImageURI(data?.data)  // 이미지 가져와서 넣음
        }
    }
}